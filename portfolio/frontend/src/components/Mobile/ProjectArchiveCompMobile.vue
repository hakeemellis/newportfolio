    ...
    setup() {
      ...
      const generateSectorTagSuggestions = async () => {
        try {
          ...
          // Ensure "All" is always the first tag
          sectorTags.value = ['All', ...generatedTags];
        } catch (error) {
      ...
      // Function to Filter Tags for Project Description - Tags are coming from adminpanel.vue (project.tags)
      ...
      // End of function to filter projects (based on selected tag)

      // Function to check if sector tag selected is similar to project tag
      const isSimilarTag = (tag, selectedTag) => {
        // Convert both tags to lowercase
        const tagLower = tag.toLowerCase();
        const selectedTagLower = selectedTag.toLowerCase();

        // If the selected tag is the same as the project tag, return true
        if (
          tagLower.includes(selectedTagLower) ||
          selectedTagLower.includes(tagLower)
        ) {
          return true;
        }

        // Handle common synonyms or variations of tags
        const synonyms = {
        ...
        for (const [key, values] of Object.entries(synonyms)) {
          // If the selected tag is similar to the key or the key is similar to the selected tag
          if (
            (tagLower === key.toLowerCase() && values.includes(selectedTag)) ||
            (values.includes(tag) && selectedTagLower === key.toLowerCase())
          ) {
            return true;
          }
        }
      ...
      // End of function to check if sector tag selected is similar to project tag
